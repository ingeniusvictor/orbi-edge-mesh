#!/usr/bin/env python3
"""ORBI Edge Manager v0.2 prototype.
...
def evict_stale(registry: NodeRegistry, ttl_seconds: float) -> None:
    for node in list(registry.all()):
        if seconds_since(node.last_seen) > ttl_seconds:
            print(
                f"[EVICT] {node.node_id} stale for "
                f"{seconds_since(node.last_seen):.1f}s"
            )
...
def route_with_fallback(
    ...
    for _ in range(max_attempts):
        node = choose_node(registry.all(), service, attempted)
        if node is None:
            break
        route_order.append(node.node_id)
